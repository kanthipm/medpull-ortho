...
class Observation(Base):
    ...
    # The patient-local calendar day, materialized at ingest
    # (connectors/base.local_date_of) so no reader has to call
    # func.date(start_time): a West Coast patient's evening activity must not
    # land on the next UTC day. This column is the product's single day
    # definition — the engine builds its post-op day axis from it
    # (engine/dataload.py) whatever tz an observation arrives in.
    ...
    # would silently rewrite history on recompute.
