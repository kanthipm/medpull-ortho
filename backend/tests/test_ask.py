...
def test_ask_sleep_finds_linda(db):
    result = ask(db, "Who is sleeping badly?")
    assert "linda" in result["patient_ids"]
    assert "Linda" in result["answer"]
...
def test_ask_endpoint(client):
    response = client.post("/api/ask", json={"question": "Who is sleeping badly this week?"})
    assert response.status_code == 200
    body = response.json()
    assert "linda" in body["patient_ids"]
    assert client.post("/api/ask", json={"question": "hi"}).status_code == 422


def test_draft_message_endpoint(client):
    body = client.post("/api/patients/robert/actions/draft-message").json()
    assert 20 <= len(body["message"]) <= 320
    assert not BANNED.search(body["message"])
    assert "Robert" in body["message"]
    # activity decline is robert's top signal — the fallback draft asks about walking
    assert "walking" in body["message"].lower()
...
def test_render_block_carries_evidence_the_verifier_needs(db):
    ...
    block = _render_block(roster["robert"])
    header, *lines = block.splitlines()
    assert header.startswith("PATIENT id=robert | Robert Hale")
    assert "priority: medium" in header
...
def test_validate_ask_keeps_only_real_ids_and_clean_prose():
    valid = {"robert", "james"}
    ok = _validate_ask(
        {"answer": "  Robert is the one to look at.  ", "patient_ids": ["robert", "ghost"]},
        valid,
    )
    assert ok == {"answer": "Robert is the one to look at.", "patient_ids": ["robert"]}
    ...
    assert _validate_ask(
        {"answer": f"Robert is stable. {GUARDRAIL_SENTENCE}", "patient_ids": []}, valid
    ) is not None
...
def test_ask_llm_falls_back_to_verified_evidence_when_composing_fails(
    ...
    llm = FakeLLM(
        retrieved={"answer": "Robert.", "patient_ids": ["robert"]},
        verdicts={"robert": {"match": True, "evidence": "resting HR rising vs baseline"}},
        composed=None,  # the composer is unavailable
    )
    monkeypatch.setattr(ask_mod, "complete_json", llm)
    result = ask_mod._ask_llm("Who has a rising heart rate?", _roster_context(db), {"robert"})
    assert result["patient_ids"] == ["robert"]
    assert result["answer"] == "1 match: Robert Hale — resting HR rising vs baseline"
...
def test_ask_llm_drops_a_candidate_whose_evidence_is_diagnostic(
    ...
    llm = FakeLLM(
        retrieved={"answer": "Robert.", "patient_ids": ["robert"]},
        verdicts={"robert": {"match": True, "evidence": "wound infection detected"}},
        composed={"answer": "Robert Hale needs a call."},
    )
    monkeypatch.setattr(ask_mod, "complete_json", llm)
    result = ask_mod._ask_llm("Who has an infection?", _roster_context(db), {"robert"})
    assert result["patient_ids"] == []
...
def test_ask_does_not_cache_an_outage_as_a_finding(db, groq_configured, monkeypatch):
    ...
    question = "Who is behind on recovery right now?"

    def outage(system, user, **kwargs):
        if system is ASK_SYSTEM:
            return {"answer": "Robert.", "patient_ids": ["robert"]}
    ...
    assert "robert" in during["patient_ids"]

    groq_configured._cooldowns["groq"] = 0.0  # the provider recovers
    healthy = FakeLLM(
        retrieved={"answer": "Robert.", "patient_ids": ["robert"]},
        verdicts={"robert": {"match": True, "evidence": "skin temperature elevated vs baseline"}},
        composed={"answer": "Robert Hale: skin temperature elevated vs baseline."},
    )
    ...
    assert after["answer"] == "Robert Hale: skin temperature elevated vs baseline."
...
def test_a_partial_outage_is_never_answered_as_a_finding(db, groq_configured, monkeypatch):
    ...
    match: Robert" while two patients were never looked at. The old guard
    ...
    def partial(system, user, **kwargs):
        if system is ASK_SYSTEM:
            return {"answer": "Several.", "patient_ids": ["robert", "linda", "sofia"]}
        if system is VERIFY_SYSTEM:
            seen.append(user)
            if len(seen) == 1:
        ...
        return {"answer": "Robert Hale: skin temperature elevated."}
    ...
    assert provider_mod.provider_name() == "groq"
    assert result["provider"] == "fallback"
    assert result["answer"] != "Robert Hale: skin temperature elevated."
...
def test_a_rotation_of_questions_stays_cached(db, groq_configured, monkeypatch):
    ...
    llm = FakeLLM(
        retrieved={"answer": "Robert.", "patient_ids": ["robert"]},
        verdicts={"robert": {"match": True, "evidence": "skin temperature elevated"}},
        composed={"answer": "Robert Hale: skin temperature elevated."},
    )
    monkeypatch.setattr(ask_mod, "complete_json", llm)
