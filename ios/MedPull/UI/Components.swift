...
struct EmptyRow: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            Image(systemName: icon).font(.mp(26)).foregroundStyle(MP.faint)
            Text(title).font(.mp(15, weight: .semibold)).foregroundStyle(MP.ink)
            if let detail {
                Text(detail).font(.mp(13)).foregroundStyle(MP.muted)
...
struct ErrorBanner: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 8) {
            ...
            Text(text).font(.mp(13.5, weight: .medium)).foregroundStyle(MP.ink)
        }
        .padding(12)
        .frame(maxWidth: .infinity, alignment: .leading)
